...
namespace Ddalgak
{
    public class EffectManagement : EditorWindow
    {
        ...
        [MenuItem("Game Utility/Effect Management")]
        ...
        private void DrawSceneSection()
        {
            ...
            var categoryGroups = filteredEntries
                .GroupBy(entry => entry.debugAttr.Category)
        ...
        private void DrawSceneCategory(string category, IReadOnlyList<ManagedEffectEntry> entries)
        {
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                var groupedEntries = entries
                    .Where(entry => entry.targetObject != null)
                    .GroupBy(entry => entry.targetObject.GetType())
                    .OrderBy(group => group.First().debugAttr.Order)
                    .ThenBy(group => group.First().debugAttr.Name);
        ...
        private void DrawSceneEntry(IReadOnlyList<ManagedEffectEntry> entries)
        {
            ...
            if (representative.targetObject == null)
            {
                return;
            }

            var label = entries.Count > 1 ? $"{representative.debugAttr.Name} ({entries.Count})"
                                                 : representative.debugAttr.Name;
        ...
        private void DrawSceneTargetInfo(IReadOnlyList<ManagedEffectEntry> entries)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                {
                    EditorGUILayout.ObjectField(entry.targetObject.gameObject, typeof(GameObject), true);

                    if (GUILayout.Button("Select", GUILayout.Width(60f)))
                    {
                        Selection.activeGameObject = entry.targetObject.gameObject;
                        EditorGUIUtility.PingObject(entry.targetObject.gameObject);
                    }
                }
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("Select All"))
                {
                    Selection.objects = entries
                        ...
                        .Select(x => (UnityEngine.Object)x.targetObject.gameObject)
                        .ToArray();
                }

                if (GUILayout.Button("Ping Script"))
                {
                    var script = MonoScript.FromMonoBehaviour(entries[0].targetObject);
        ...
        {
            var hierarchyPath = GetHierarchyPath(entry.targetObject.transform);
            ...
            {
                EditorGUILayout.ObjectField("Attached Object", entry.targetObject.gameObject, typeof(GameObject), true);
                EditorGUILayout.ObjectField("Component", entry.targetObject, entry.targetObject.GetType(), true);
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button("Ping Script"))
                {
                    var script = MonoScript.FromMonoBehaviour(entry.targetObject);
        ...
        private void DrawPrefabCategory(string category, IReadOnlyList<ManagedEffectEntry> entries)
        {
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                var prefabGroups = entries
                    .Where(entry => entry.targetObject != null && !string.IsNullOrEmpty(entry.assetPath))
            ...
            IReadOnlyList<ManagedEffectEntry> entries)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                var localPath = GetRelativePath(prefabRoot != null ? prefabRoot.transform : null, entry.targetObject.transform);
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    {
                        EditorGUIUtility.PingObject(entry.targetObject);
                    }
                }
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button("Ping Script"))
                {
                    var script = MonoScript.FromMonoBehaviour(representative.targetObject);
        ...
        private void DrawInspector(IReadOnlyList<ManagedEffectEntry> entries)
        {
            ...
            var targets = validEntries
                .Select(x => (UnityEngine.Object)x.targetObject)
                .ToArray();
        ...
        private bool IsSearchMatch(ManagedEffectEntry entry)
        {
            ...
            return ContainsIgnoreCase(entry.debugAttr.Name, search) ||
                   ContainsIgnoreCase(entry.targetObject.GetType().Name, search) ||
                   ContainsIgnoreCase(entry.targetObject.gameObject.name, search) ||
                   ContainsIgnoreCase(entry.debugAttr.Category, search) ||
                   ContainsIgnoreCase(GetHierarchyPath(entry.targetObject.transform), search) ||
        ...
        private static int GetDistinctEffectCount(IReadOnlyList<ManagedEffectEntry> entries)
        {
            return entries
                .Where(x => x.targetObject != null)
                .Select(x => x.targetObject.GetType())
                .Distinct()
                .Count();
        }
